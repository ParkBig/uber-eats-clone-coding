...
export class OrderService {
  ...
  async getOrders(user: User, { status }: GetOrdersInput): Promise<GetOrdersOutput> {
    try {
      ...
      if (user.role === UserRole.Client) {
        ...
        const restaurant = await this.restaurants.find({ where: { owner: { id: user.id } }, relations: ["order"] });
        orders = restaurant.map(restaurant => restaurant.order).flat(1);
  ...
  async getOrder(user: User, { id }: GetOrderInput): Promise<GetOrderOutput> {
    try {
      ...
      if (!this.canSeeOrder(user, order)) return { ok: false, error: "You can't see that." };
  ...
  async editOrder(user: User,  { id, status }: EditOrderInput): Promise<EditOrderOutput> {
    try {   
      const order = await this.orders.findOne({ where: { id }});
      if (!order) return { ok: false, error: "Order not found." };
      if (!this.canSeeOrder(user, order)) return { ok: false, error: "Can't see this." };
      let canEdit = true;
      if (user.role === UserRole.Client) {
        canEdit = false;
      };
  ...
  async takeOrder(driver: User, { id }: TakeOrderInput): Promise<TakeOrderOutput> {
    try {
      const order = await this.orders.findOne({ where: { id } });
      if (!order) return { ok: false, error: "Order not found." }
      if (order.driver) return { ok: false, error: "This order already has a driver." }
      await this.orders.save([{ id, driver }]);
      await this.pubsub.publish(NEW_ORDER_UPDATE, { orderUpdates: { ...order, driver, droverId: driver.id } });
      return { ok: true };
    } catch (error) {
